...
def emb_lookup_multi_emb_size(input_tensor, input_tensor_feat_min, emb_size_config, input_feat_num):
    input_emb_tensors = []
    print("current config: ",emb_size_config)
    for i in range(len(input_tensor_feat_min)):
        if(emb_size_config[i]==0):
            continue
        cur_input = input_tensor[:, i:i+1] # bs*1
        cur_feat_min = input_tensor_feat_min[i]
        cur_real_input = tf.reshape(cur_input - cur_feat_min, [-1])
        cur_input_feat_num =  input_feat_num[i]
        
        cur_full_emb = get_variable(name='full_emb_for_{}_emb_size_{}'.format(i,emb_size_config[i]), \
                                    shape=[cur_input_feat_num,emb_size_config[i]])
        cur_emb = tf.gather(cur_full_emb, cur_real_input)
        input_emb_tensors.append(cur_emb)
    return input_emb_tensors
...
def normal_mlp(init, layer_sizes, layer_acts, h, node_in, batch_norm=False, layer_norm=False, training=True):
    layer_kernels = []
    layer_biases = []
